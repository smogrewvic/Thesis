...
def plot_points(ego_data):
    data = []
    titles = []
    for key in Spawn_Points.pedestrian_points.value:
        x = str(round(Spawn_Points.pedestrian_points.value[key][0], 7))
        y = str(round(Spawn_Points.pedestrian_points.value[key][1], 7))
        z = str(round(Spawn_Points.pedestrian_points.value[key][2], 7))
        pitch = str(round(Spawn_Points.pedestrian_points.value[key][3], 7))
        roll = str(round(Spawn_Points.pedestrian_points.value[key][4], 7))
        yaw = str(round(Spawn_Points.pedestrian_points.value[key][5], 7))
        ...
        titles.append(name)
        data.append(Spawn_Points.pedestrian_points.value[key])
    ...
    fig_spawn_points = px.scatter(x=-data[:, 0],  # flip x-axis display
                                  ...
                                  hover_name=titles,
                                  range_color=[0, 2],
                                  title="Spawn Points, use Spawn_Points.pedestrian_points.value['id_VALUE']")
